...
import os
'''
load data and infection tree logic is identical to viterbi
'''
...
def load_data(data_dir="paper_data"):
    try:
        counties_gpd = gpd.read_file(os.path.join(data_dir, "map/map/cbg/la_cbg.shp"))
        df_probs = pd.read_csv(os.path.join(data_dir, "la_cbg.csv"))
        df_stats = pd.read_csv("us_county_reporting_probs.csv")
        df_la_true = pd.read_csv(os.path.join(data_dir, "LA_simu/LA_simu_1_truth.csv"))
        df_la = pd.read_csv(os.path.join(data_dir, "LA_simu/LA_simu_1_test.csv"))
    ...
    for i, row in df_la.iterrows():
        ...
        if i == 0:   
            ...
            for _, row in parents_nodes.iterrows():    
                child_node, source_geoid, target_geoid = row['Target'], row['from_geoid'], row['to_geoid']
                infector_key = f"{source_geoid}-{parent_id}"
                ...
                real_tree[infector_key].append((infected_key, 1, obs))
        else:
            for _, row in parents_nodes.iterrows():   
                child_node, source_id, target_geoid = row['Target'], row['from_geoid'], row['to_geoid']
                obs = target_geoid != '???'
                infector_key = f"{source_geoid}-{parent_id}"
                child_id = getints(child_node)
                infected_key = f"{target_geoid}-{child_id}"
...
def setup_matrices(df_probs, df_stats):
    transition_matrix = pd.read_csv("paper_data/transition_matrix_sim.csv")
...
def create_tree(df_la):
    real_tree = defaultdict(list)
    
    for i, row in df_la.iterrows(): #will need a check to not include duplicates in dict
        parent_node, source_geoid=   row['Source'], row['from_geoid']
        parent_id = getints(parent_node)
        infector_key = f"{source_geoid}-{parent_id}"
        parents_nodes = df_la.loc[df_la['Source'] == parent_node]    
        if infector_key in real_tree: #might only allow one child per parent check this
            continue
        if i == 0:   
            #print(f"The initial case is: {parent_id}")
            #print(f"The other parents of this case are: {parents_nodes}")
            for _, row in parents_nodes.iterrows():    
                child_node, source_geoid, target_geoid = row['Target'], row['from_geoid'], row['to_geoid']
                infector_key = f"{source_geoid}-{parent_id}"
                child_id = getints(child_node)
                infected_key = f"{target_geoid}-{child_id}"
                #print(f" The first infected children are: {infected_key}")
                if target_geoid == '???': obs = False
                else: obs = True
                real_tree["initial"].append((infector_key, 0, True)) 
                real_tree[infector_key].append((infected_key, 1, obs)) 
        else:
            if (i % 1000 == 0): print(f"\n Creating infection tree: {(i/len(df_la))*100:.3f}% done")
            for _, row in parents_nodes.iterrows():   
                child_node, source_id, target_geoid = row['Target'], row['from_geoid'], row['to_geoid']
                if target_geoid == '???': obs = False
                else: obs = True
...
class TreeNode:
    def __init__(self, id, county, person_id, day=None, parent=None, observed=False):
        ...
        self.guess = [] #viterbi uses this
        self.baseline = [] #linear interpolation uses this, -1 if CBG not found
        self.constant = [] #copy of parent 
...
def random_path(vpath, counties, county_index, transition_matrix, p_results=False):
    '''
    takes a random sample from transition matrix
    '''    
    ...
    for pos in range(n_unobserved):
        ...
        print(f"Parent county of {curr_node}: {parent_county}")
...
def viterbi(vpath, counties, county_index, transition_matrix, emission_probs, counties_gpd, debug=False, p_results=False):
    ...
    print(f"Looking for counties: {first_ob.county}, {last_ob.county}")
    ...
    log_transition_matrix = (np.log(transition_matrix + 1e-10)).values
    ...
    for pos in range(n_unobserved):
        ...
        if unobserved_linear_interpolation:
            actual_node.baseline = unobserved_linear_interpolation[pos]
        else:
            actual_node.baseline = -1 
...
'''
 ...
 It does find paths that are in the format: 
    ...
    NOTE*** in the data, this is already structured this way, for both real world and simulation.
    however, this is not a requirement if you want to run on new data, this will find the revelant paths for you
...
and inserts it into the relevent row/column in df_la. It also attempts to measure accuracy
'''
...
def main():
    ...
    parser.add_argument('--output', default='paper_data/imputed_sim.csv')
    ...
    if RANDOM_FLAG:
        ...
        startsearch(real_infection_tree, counties, county_index, transition_matrix, emission_probs, counties_gpd)
